...
from .redis_store import get_async_redis
...
class RedisRateLimiter:
    ...
    if tokens >= 1 then
        tokens = tokens - 1
        reset = (limit - tokens) / refill_rate
        redis.call('HMSET', KEYS[1], 'tokens', tokens, 'last', now)
        redis.call('EXPIRE', KEYS[1], ttl)
        return {1, tokens, reset}
    else
        local deficit = 1 - tokens
        reset = deficit / refill_rate
        redis.call('HMSET', KEYS[1], 'tokens', tokens, 'last', now)
        redis.call('EXPIRE', KEYS[1], ttl)
        return {0, tokens, reset}
    end
    """
    ...
    async def dispatch(self, request: Request, call_next):  # type: ignore[override]
        ...
        identifier = self._fallback._identifier_for(request)
        try:
            allowed, remaining, reset_in = await self._consume(identifier, limit, window)
        except Exception as exc:  # pragma: no cover - redis down
            logging.getLogger(__name__).warning("Redis rate limiter fallback: %s", exc)
            return await self._fallback.dispatch(request, call_next)
    ...
    async def _consume(self, identifier: str, limit: int, window: int):
        assert self._redis is not None
        now = time.time()
        key = f"rate:{identifier}"
        args = [str(limit), str(window), f"{now:.6f}"]
        try:
            if not self._script_sha:
                self._script_sha = await self._redis.script_load(self.LUA_SCRIPT)
            result = await self._redis.evalsha(self._script_sha, 1, key, *args)
        except NoScriptError:
            self._script_sha = await self._redis.script_load(self.LUA_SCRIPT)
            result = await self._redis.evalsha(self._script_sha, 1, key, *args)
        allowed, tokens, reset_in = result
        return int(allowed), float(tokens), float(reset_in)
