...
class UltimateBackendAudit:
    ...
    def run(self) -> List[StepResult]:
        ...
        for scenario in scenarios:
            ...
            try:
                ...
                self.results.append(
                    StepResult(label, False, f"Unexpected error: {exc!r}")
                )
            else:
    ...
    def health_and_ui_surfaces(self) -> str:
        ...
        self._assert(
            health.status_code == 200, f"/health returned {health.status_code}"
        )
        body = health.json()
        ...
        self._assert(
            openapi.status_code == 200, f"/openapi.json returned {openapi.status_code}"
        )

        root = self.client.get("/", follow_redirects=False)
        self._assert(
            root.status_code in (307, 308),
            f"/ root expected redirect but got {root.status_code}",
        )
        self._assert(
    ...
    def restaurant_catalog_and_queries(self) -> str:
        ...
        self._assert(
            detail.status_code == 200, f"/restaurants/{{id}} {detail.status_code}"
        )
        ...
        self._assert(
            ...
            f"/restaurants/{{id}}/floorplan {floorplan.status_code}",
        )

        query_name = self.client.get(
            "/restaurants", params={"q": target["name"].split()[0]}
        )
        self._assert(
            any(rest["id"] == target["id"] for rest in query_name.json()),
            "name query failed",
        )

        query_city = self.client.get(
            "/restaurants", params={"q": target.get("city", "")[:3]}
        )
        self._assert(query_city.json(), "city query returned empty results")

        query_cuisine = self.client.get(
            "/restaurants", params={"q": (target.get("cuisine") or [""])[0]}
        )
    ...
    def availability_consistency(self) -> str:
        ...
        self._assert(
            ...
            f"availability returned {availability.status_code}",
        )
        slots = availability.json()["slots"]
        self._assert(slots, "availability returned no slots")
        for slot in slots:
            self._assert(
                slot["count"] == len(slot["available_table_ids"]), "slot count mismatch"
            )
    ...
    def reservation_lifecycle_and_conflicts(self) -> str:
        ...
        self._assert(
            created.status_code == 201, f"create returned {created.status_code}"
        )
        ...
        eligible_capacities = [
            ...
            if t["capacity"] >= autopick_payload["party_size"]
        ]
        self._assert(eligible_capacities, "no eligible tables found for autopick")
        chosen_capacity = tables[table_id]["capacity"]
        self._assert(
            chosen_capacity == min(eligible_capacities),
            "autopick chose non-minimal table",
        )
        ...
        return (
            "conflicts, cancel/confirm, delete, and autopick minimal-selection enforced"
        )
    ...
    def reservation_listing_and_status_guards(self) -> str:
        ...
        self._assert(
            any(item["id"] == resid for item in listing.json()),
            "reservation missing from list",
        )
        ...
        try:
            ...
            invalid_status = exc
        self._assert(
            invalid_status is not None, "invalid status should raise HTTPException"
        )
    ...
    def cancellation_without_delete_restores_availability(self) -> str:
        ...
        slots = availability.json()["slots"]
        target = next(
            (slot for slot in slots if slot["start"].endswith("T12:00:00")), None
        )
        ...
        during_slot = next(
            slot for slot in during.json()["slots"] if slot["start"] == target["start"]
        )
        self._assert(
        ...
        after_slot = next(
            slot for slot in after.json()["slots"] if slot["start"] == target["start"]
        )
        self._assert(
            after_slot["count"] == target["count"], "count should restore after cancel"
        )
    ...
    def persistence_roundtrip(self) -> str:
        ...
        listing = self.client.get("/reservations")
        self._assert(
            any(item["guest_name"] == "PersistStrict" for item in listing.json()),
            "persisted reservation missing",
        )
        return "reservations persist to SQLite and survive reload"


def run_command(
    cmd: Sequence[str], cwd: Path, name: str, summary: List[StepResult]
) -> None:
    try:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the ultimate Baku Reserve test harness."
    )
    parser.add_argument(
        "--skip-pytest", action="store_true", help="Skip backend pytest invocation"
    )
    parser.add_argument(
        "--skip-mobile", action="store_true", help="Skip mobile Jest suite"
    )
    parser.add_argument(
        "--skip-tsc", action="store_true", help="Skip TypeScript type checks"
    )
